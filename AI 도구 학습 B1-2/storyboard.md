# 🎬 무신사 스탠다드 AI 광고 영상 스토리보드 기획서

**프로젝트명:** 무신사 스탠다드 AI 광고 영상  
**제작일:** 2025년  
**제작 도구:** Leonardo.ai GPT Image 2 / Hailuo 2.3  
**총 영상 길이:** 12초 (6초 × 2컷)

---

## 📌 브랜드 아이덴티티

| 항목 | 내용 |
|------|------|
| 브랜드 | 무신사 스탠다드 (MUSINSA STANDARD) |
| 컨셉 | 일상 속 자연스러운 스타일, 심플하고 모던한 감성 |
| 타겟 | 20~30대 패션에 관심 있는 남녀 |
| 톤앤매너 | 미니멀, 세련됨, 자연스러움 |
| 키워드 | #베이직 #데일리룩 #심플 #모던 |

---

## 🎯 광고 목표

> **"입는 순간, 일상이 달라진다"**  
> 무신사 스탠다드의 베이직 아이템이 일상을 얼마나 세련되게 바꿔주는지 시각적으로 전달

---

## 🎞️ 씬 구성 (총 2컷)

---

### 🎬 씬 1 — 도심 속 자연스러운 등장 (0~6초)

| 항목 | 내용 |
|------|------|
| 장면 | 도심 거리, 자연광 아래 모델이 걸어오는 장면 |
| 의상 | 무신사 스탠다드 베이직 아우터 + 슬랙스 |
| 분위기 | 밝고 깔끔한 도시 감성, 자연스러운 무드 |
| 카메라 | 정면 슬로우 워킹 샷 |
| 이미지 생성 | Leonardo.ai GPT Image 2 |
| 영상 변환 | Hailuo 2.3 (6초) |
| 해상도 | 1376×768 (768p) |

**프롬프트 요약:**  
`person walking slowly forward, cinematic camera movement, smooth natural motion, minimal style, soft lighting`

---

### 🎬 씬 2 — 클로즈업 & 브랜드 강조 (6~12초)

| 항목 | 내용 |
|------|------|
| 장면 | 의상 디테일 클로즈업 후 브랜드 로고 페이드인 |
| 의상 | 무신사 스탠다드 흰 티셔츠 |
| 분위기 | 차분하고 세련된 스튜디오 감성 |
| 카메라 | 티셔츠 클로즈업 |
| 이미지 생성 | Leonardo.ai GPT Image 2 |
| 영상 변환 | Hailuo 2.3 (6초) |
| 해상도 | 1376×768 (768p) |

**프롬프트 요약:**  
`slow zoom in on white clothing, soft light, clean white background, cinematic product shot, minimal`

---

## 🛠️ 제작 도구 및 워크플로우
[기획] → [프롬프트 작성] → [Leonardo.ai로 이미지 생성]
→ [Hailuo 2.3으로 영상 변환] → [최종 편집 및 완성]
| 단계 | 도구 | 역할 |
|------|------|------|
| 이미지 생성 | Leonardo.ai GPT Image 2 | 광고 장면 이미지 제작 |
| 영상 변환 | Hailuo 2.3 | 이미지 → 6초 영상 변환 |
| 해상도 | 1376×768 | 광고용 와이드 포맷 |

---

## 📋 스토리보드 요약표

| 씬 | 시간 | 장소 | 의상 | 분위기 | 도구 |
|----|------|------|------|--------|------|
| 씬 1 | 0~6초 | 도심 거리 | 흰 티셔츠 + 베이지 슬랙스 | 밝고 자연스러운 도시 감성 | Leonardo + Hailuo |
| 씬 2 | 6~12초 | 스튜디오 | 흰 티셔츠 | 차분하고 세련된 감성 | Leonardo + Hailuo |

---

## ✅ 최종 산출물

- [x] 이미지 2장 생성 완료 (Leonardo.ai GPT Image 2)
- [x] 영상 2개 변환 완료 (Hailuo 2.3, 각 6초)
- [x] 총 영상 길이 12초 달성
- [x] 브랜드 컨셉 반영 완료
- [x] 스토리보드 기획서 작성 완료

---

*© 2025 무신사 스탠다드 AI 광고 프로젝트*
